/* 
Question URL : http://www.codewars.com/kata/partition-on
...
function partitionOn(pred, items) {
    var False_array = items.filter( function(number) { return !pred(number); } ); 
    var True_array = items.filter(pred); 
    items.length = 0;
    for(var i = 0; i < False_array.length; i++) 
    { 
        items.push(False_array[i]); 
    }
    for(var i = 0; i < True_array.length; i++)
    { 
        items.push(True_array[i]);
    }
    console.log(items);
    return False_array.length;
  }
